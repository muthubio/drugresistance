...
def _add_source_to_info(info_field: str, source: str) -> str:
    ...
    if f"SOURCE={source}" in info_field:
        return info_field
...
def merge_vcfs(vcf_sources, out_vcf: str):
    ...
    if not existing:
        ...
        return False, {"total_records": 0, "deduplicated_records": 0, "gatk_records": 0, "delly_records": 0}

    if len(existing) == 1:
        src_path, src_name = existing[0]
        if os.path.abspath(src_path) != os.path.abspath(out_vcf):
            try:
                shutil.copyfile(src_path, out_vcf)
            except Exception as e:
                logging.error(f"Failed to copy single VCF {src_path} -> {out_vcf}: {e}")
                return False, {"total_records": 0, "deduplicated_records": 0, "gatk_records": 0, "delly_records": 0}
        stats = {
            "total_records": 0,
            "deduplicated_records": 0,
            "gatk_records": 0 if src_name != "GATK" else 1,
            "delly_records": 0 if src_name != "DELLY" else 1,
        }
        logging.info(f"Single {src_name} VCF used as final output: {out_vcf}")
        return True, stats
    ...
    for idx, (vcf, source) in enumerate(existing):
        try:
            with _open_maybe_gzip(vcf, 'rt') as fh:
                for line in fh:
                    if line.startswith('#'):
                        if line.startswith('##'):
                            if line.startswith('##INFO=<ID=SOURCE,'):
                                if line not in seen_headers:
                                    merged_headers.append(line)
                                    seen_headers.add(line)
                            elif idx == 0 and line not in seen_headers:
                                merged_headers.append(line)
                                seen_headers.add(line)
                        elif line.startswith('#CHROM') and chrom_header is None:
                            chrom_header = line
                        continue
                    ...
                    if key not in variant_map:
                        ...
                        existing_line = variant_map[key].rstrip('\n').split('\t')
                        new_line = normalized.rstrip('\n').split('\t')
                        existing_info = existing_line[7] if len(existing_line) > 7 else '.'
                        new_info = new_line[7] if len(new_line) > 7 else '.'
                        if 'SOURCE=' in existing_info and 'SOURCE=' in new_info:
                            sources = set()
                            for info in [existing_info, new_info]:
                                for item in info.split(';'):
                                    if item.startswith('SOURCE='):
                                        sources.update(item.replace('SOURCE=', '').split(','))
                            merged_source = 'SOURCE=' + ','.join(sorted(sources))
                            base_info = [x for x in existing_info.split(';') if not x.startswith('SOURCE=')]
                            if base_info:
                                existing_line[7] = ';'.join(base_info + [merged_source])
                            else:
                                existing_line[7] = merged_source
                            variant_map[key] = '\t'.join(existing_line) + '\n'
        except Exception as e:
            logging.error(f"Failed to read VCF {vcf}: {e}")
            return False, {"total_records": 0, "deduplicated_records": 0, "gatk_records": 0, "delly_records": 0}

    if not any(h.startswith('##INFO=<ID=SOURCE,') for h in merged_headers):
        merged_headers.append('##INFO=<ID=SOURCE,Number=.,Type=String,Description="Variant caller source used during smart merge (GATK, DELLY, or both)">\n')
    ...
    try:
        ...
        logging.info(
            f"Smart merged VCFs into {out_vcf} | total={stats['total_records']} unique={stats['deduplicated_records']} "
...
class ProgressBar(Flowable):
    def __init__(self, label, value, width=54*mm, height=6*mm):
        Flowable.__init__(self)
        self.label = label
    ...
    def draw(self):
        ...
        c.roundRect(0, 0, self.width, self.height, 1.2*mm, stroke=0, fill=1)
        fill_w = self.width * (self.value / 100.0)
        c.setFillColor(colors.HexColor("#2563EB"))
        c.roundRect(0, 0, fill_w, self.height, 1.2*mm, stroke=0, fill=1)
...
def build_visualization_panel(top_lineages, merge_stats):
    bars = [ProgressBar(label, value) for label, value in top_lineages]
    left_tbl = Table([[b] for b in bars], colWidths=[58*mm]) if bars else Table([[Paragraph("No lineage visualization available", getSampleStyleSheet()['Normal'])]], colWidths=[58*mm])
    left_tbl.setStyle(TableStyle([('LEFTPADDING', (0,0), (-1,-1), 2), ('RIGHTPADDING', (0,0), (-1,-1), 2)]))

    stats_rows = [
        ["VCF merge metric", "Count"],
        ["Total input records", str(merge_stats.get('total_records', 0))],
        ["Unique merged records", str(merge_stats.get('deduplicated_records', 0))],
        ["GATK records", str(merge_stats.get('gatk_records', 0))],
        ["DELLY records", str(merge_stats.get('delly_records', 0))],
    ]
    right_tbl = Table(stats_rows, colWidths=[52*mm, 22*mm], repeatRows=1)
    ...
    panel = Table([[left_tbl, right_tbl]], colWidths=[64*mm, 78*mm])
...
def generate_infographic_pdf(sample_id: str, predicted: str, mixed: str, category: str,
                             ...
                             matched_res_csv: str, output_pdf: str, merge_stats: dict):
    ...
    top_lineages = sorted(results.items(), key=lambda x: x[1], reverse=True)[:5]
    ...
    story.append(build_visualization_panel(top_lineages, merge_stats))
    ...
    story.append(Spacer(1, 4*mm))
    story.append(Paragraph("Smart merge tags variants by caller source and summarizes merge statistics in this report.", normal))
    doc.build(story)
...
def process_sample(sample_path, ref_genome, output_dir, lineage_references,
    ...
    merge_stats = {"total_records": 0, "deduplicated_records": 0, "gatk_records": 0, "delly_records": 0}

    if vcf_override and Path(vcf_override).exists():
        vcf_lineage = str(Path(vcf_override))
        vcf_resist_final = str(Path(vcf_override))
        logging.info(f"[{sample_id}] Using provided VCF for both lineage and resistance: {vcf_override}")
    else:
        if bam_override and Path(bam_override).exists():
            ...
            if kind == "vcf":
                ...
                logging.info(f"[{sample_id}] Detected VCF input")
            elif kind == "bam":
                sorted_bam = inputs[0]
                logging.info(f"[{sample_id}] Detected BAM input")
            else:
        ...
        if sorted_bam and (not vcf_lineage or not vcf_resist_final):
            ...
            if not run_cmd(
                f"gatk HaplotypeCaller -R {q(ref_genome)} -I {q(sorted_bam)} -O {q(vcf_lineage)} -L {q(tmp_bed)} --sample-ploidy 1"
            ):
                return

            vcf_resist_gatk = str(sample_dir / f"{sample_id}.resist.gatk.vcf")
            if not run_cmd(
                f"gatk HaplotypeCaller -R {q(ref_genome)} -I {q(sorted_bam)} -O {q(vcf_resist_gatk)} --sample-ploidy 1"
            ):
                return
            ...
            ok, merge_stats = merge_vcfs([
                (vcf_resist_gatk, "GATK"),
                (vcf_resist_delly, "DELLY")
            ], vcf_resist_final)
            if not ok:
    ...
    with open(lin_out, 'w') as f:
        ...
        for lineage in sorted(lineage_references.keys()):
            f.write(f"{lineage:<20}{results.get(lineage, 0.0):.2f}%{'':<9}{matched_counts.get(lineage, '0/0')}\n")
        f.write("\n" + "=" * 60 + "\n")
...
def main():
    ap = argparse.ArgumentParser(
        description="LineageXpress v10: smart source-aware merge plus visualization-rich A3 PDF",
