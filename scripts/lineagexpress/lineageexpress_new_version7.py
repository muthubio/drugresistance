...
# -------------------- infographic helpers ----------------------
class FlaskIcon(Flowable):
    def __init__(self, label, solution_color=colors.HexColor("#4F46E5"), width=34*mm, height=42*mm, subtitle=""):
    ...
    def draw(self):
        ...
        c.rect(cx - neck_w/2, base_y + body_h, neck_w, neck_h, stroke=1, fill=0)

        path = c.beginPath()
        path.moveTo(cx - body_top_w/2, base_y + body_h)
        path.lineTo(cx - body_bot_w/2, base_y)
        path.lineTo(cx + body_bot_w/2, base_y)
        path.lineTo(cx + body_top_w/2, base_y + body_h)
        path.close()
        c.drawPath(path, stroke=1, fill=0)

        liquid_path = c.beginPath()
        liquid_path.moveTo(cx - body_bot_w/2 + 2, base_y + 2)
        liquid_path.lineTo(cx + body_bot_w/2 - 2, base_y + 2)
        liquid_path.lineTo(cx + body_top_w/2 - 3, base_y + body_h * 0.46)
        liquid_path.lineTo(cx - body_top_w/2 + 3, base_y + body_h * 0.40)
        liquid_path.close()
        c.setFillColor(self.solution_color)
        c.setStrokeColor(self.solution_color)
        c.drawPath(liquid_path, stroke=0, fill=1)

        c.setFillColor(colors.white)
        c.setStrokeColor(colors.white)
        c.circle(cx + body_bot_w*0.14, base_y + body_h*0.20, 1.7, stroke=0, fill=1)
        c.circle(cx + body_bot_w*0.05, base_y + body_h*0.28, 1.2, stroke=0, fill=1)

        c.setFillColor(colors.HexColor("#0f172a"))
        c.setStrokeColor(colors.HexColor("#0f172a"))
        c.setFont("Helvetica-Bold", 9)
        c.drawCentredString(cx, 4, self.label[:18])
        if self.subtitle:
            c.setFont("Helvetica", 7)
            c.drawCentredString(cx, -5, self.subtitle[:24])
        c.restoreState()
...
def make_summary_cards(sample_id, predicted, mixed, category):
    ...
    title_style = ParagraphStyle(
        'TitleCard', parent=styles['Heading1'], fontName='Helvetica-Bold',
        fontSize=20, leading=24, textColor=colors.white, alignment=TA_LEFT
    )
    label_style = ParagraphStyle(
        'LabelCard', parent=styles['Normal'], fontName='Helvetica',
        fontSize=8.5, leading=10, textColor=colors.white, alignment=TA_LEFT
    )
    value_style = ParagraphStyle(
        'ValueCard', parent=styles['Normal'], fontName='Helvetica-Bold',
        fontSize=12, leading=14, textColor=colors.white, alignment=TA_LEFT
    )

    left = Table([[Paragraph(f"Sample {sample_id}", title_style)]], colWidths=[82*mm])
    ...
    def card(title, value, bg):
        t = Table([[Paragraph(title, label_style)], [Paragraph(value, value_style)]], colWidths=[44*mm])
    ...
    ]], colWidths=[84*mm, 46*mm, 46*mm, 46*mm])
...
def build_lineage_table(lineage_references, results, matched_counts):
    styles = getSampleStyleSheet()
    hdr = ParagraphStyle('Hdr', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=10, textColor=colors.white)
    cell = ParagraphStyle('Cell', parent=styles['Normal'], fontName='Helvetica', fontSize=9, leading=11)

    data = [[Paragraph("Lineage", hdr), Paragraph("Probability", hdr), Paragraph("Matched SNPs", hdr)]]
    for lineage in sorted(lineage_references.keys()):
        prob = f"{results.get(lineage, 0.0):.2f}%"
        match = matched_counts.get(lineage, '0/0')
        data.append([
            Paragraph(lineage, cell),
            Paragraph(prob, cell),
            Paragraph(match, cell),
        ])

    tbl = Table(data, colWidths=[56*mm, 30*mm, 30*mm], repeatRows=1)
...
def build_drug_hits_section(matched_res_csv: str):
    styles = getSampleStyleSheet()
    ...
    body = []
    body.append(Paragraph("Drug-resistance infographic", section_title))
    ...
    df = pd.read_csv(matched_res_csv)
    if df.empty:
        note = Table([[Paragraph("Sensitive strain: no resistance-associated mutations detected.", ParagraphStyle('Sens', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=12, textColor=colors.white))]], colWidths=[180*mm])
    ...
    for _, r in df.iterrows():
        ...
        flask_row.append(FlaskIcon(drug, solution_color=drug_color_map(drug), subtitle=gene))
        detail_rows.append([drug, gene, mut, conf])

    if flask_row:
        flask_tbl = Table([flask_row], colWidths=[36*mm] * len(flask_row))
    ...
    detail_tbl = Table(detail_rows, colWidths=[38*mm, 30*mm, 48*mm, 58*mm], repeatRows=1)
...
def generate_infographic_pdf(sample_id: str, predicted: str, mixed: str, category: str,
    ...
    lineage_tbl = build_lineage_table(lineage_references, results, matched_counts)
    story.append(lineage_tbl)
    story.append(Spacer(1, 6*mm))

    story.extend(build_drug_hits_section(matched_res_csv))
    story.append(Spacer(1, 4*mm))
    story.append(Paragraph("Readable infographic report generated in a consistent layout for sample-level comparison.", normal))
...
def process_sample(sample_path, ref_genome, output_dir, lineage_references,
                   ...
                   mutations_csv=None,
                   delly_cmd="delly", skip_delly=False,
    ...
    if vcf_override and Path(vcf_override).exists():
        ...
        if sorted_bam and (not vcf_lineage or not vcf_resist_final):
            ...
            if not run_cmd(
                f"gatk HaplotypeCaller -R {q(ref_genome)} -I {q(sorted_bam)} -O {q(vcf_lineage)} "
                f"-L {q(tmp_bed)} --sample-ploidy 1"
            ):
                return

            vcf_resist_gatk = str(sample_dir / f"{sample_id}.resist.gatk.vcf")
            if not run_cmd(
                f"gatk HaplotypeCaller -R {q(ref_genome)} -I {q(sorted_bam)} -O {q(vcf_resist_gatk)} "
                f"--sample-ploidy 1"
            ):
                return
            ...
            if not merge_vcfs([vcf_resist_gatk, vcf_resist_delly], vcf_resist_final):
                logging.warning(f"[{sample_id}] Could not merge GATK + Delly VCFs; using GATK resistance VCF only")
...
def main():
    ap = argparse.ArgumentParser(
        description="LineageXpress v7: robust auto-detection + Delly + colorful infographic PDF output per sample",
    ...
    ap.add_argument("--snpeff_db", default=DEFAULT_SNPEFF_DB,
                    help="snpEff DB name")
    ap.add_argument("--delly_cmd", default="delly",
                    help="Command for Delly executable")
    ap.add_argument("--skip_delly", action="store_true",
                    help="Skip Delly structural variant calling")
    ap.add_argument("--no_infographic", action="store_true",
                    help="Skip infographic PDF generation")
