...
def merge_vcfs(vcf_inputs, out_vcf: str) -> bool:
    ...
    if len(existing) == 1:
        src = existing[0]
        if os.path.abspath(src) == os.path.abspath(out_vcf):
            return True
        try:
            shutil.copyfile(src, out_vcf)
            logging.info(f"Single VCF copied to final output: {out_vcf}")
            return True
        except Exception as e:
            logging.error(f"Failed to copy single VCF {src} -> {out_vcf}: {e}")
            return False
    ...
    for idx, vcf in enumerate(existing):
        try:
    ...
    try:
        with open(out_vcf, 'w') as out:
            ...
            for rec in records:
                out.write(rec)
        logging.info(f"Merged {len(existing)} VCFs into plain-text VCF: {out_vcf}")
        return True
    except Exception as e:
...
def main():
    ap = argparse.ArgumentParser(
        description="LineageXpress v9: clean A3 infographic PDF with robust VCF merging for plain-text or gzipped inputs",
