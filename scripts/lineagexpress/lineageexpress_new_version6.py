...
def _pair_signature(name: str):
    patterns = [
        (r"^(.*)_R1(\.fastq\.gz|\.fq\.gz|\.fastq|\.fq)$", r"\1_R2\2"),
        (r"^(.*)_1(\.fastq\.gz|\.fq\.gz|\.fastq|\.fq)$",  r"\1_2\2"),
        (r"^(.*)\.1(\.fastq\.gz|\.fq\.gz|\.fastq|\.fq)$", r"\1.2\2"),
    ]
    for pat, mate_pat in patterns:
        m = re.match(pat, name)
        if m:
            prefix = m.group(1)
            return prefix
...
def _find_in_dir(dirpath: Path, sid: str):
    """Search common paired/single patterns inside a directory, then fall back to scanning contents."""
    pats = [
...
def main():
    ap = argparse.ArgumentParser(
        description="LineageXpress: robust auto-detection + lineage + mutation summary + WHO category + Delly support",
