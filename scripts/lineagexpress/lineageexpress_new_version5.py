#!/usr/bin/env python3
import os, subprocess, argparse, logging, time, re, shlex
...
# -------------------- delly helpers ----------------------------
def run_delly_call(sorted_bam: str, ref_genome: str, out_bcf: str, out_vcf: str,
                   delly_cmd: str = "delly") -> bool:
    ...
    if vcf_override and Path(vcf_override).exists():
        ...
        if bam_override and Path(bam_override).exists():
            ...
            if kind == "vcf":
                ...
                if not run_cmd(f"samtools view -S -b {q(sam_file)} -o {q(bam_file)}"): return
                if not run_cmd(f"samtools sort -@ {threads} {q(bam_file)} -o {q(sorted_bam_path)}"): return
...
def main():
    ap = argparse.ArgumentParser(
        description="LineageXpress: lineage + mutation summary + WHO category + Delly support",
