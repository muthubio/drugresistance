...
def _add_source_to_info(info_field: str, source: str) -> str:
    info_field = (info_field or ".").strip()
    if info_field in ("", "."):
        return f"SOURCE={source}"
    if "SOURCE=" not in info_field:
        return f"{info_field};SOURCE={source}"
    items = []
    sources = set()
    for x in info_field.split(';'):
        if x.startswith('SOURCE='):
            sources.update(x.replace('SOURCE=', '').split(','))
        else:
            items.append(x)
    sources.add(source)
    items.append('SOURCE=' + ','.join(sorted(sources)))
    return ';'.join(items)


def _normalize_variant_line(line: str, source: str) -> str:
    parts = line.rstrip("\n").split("\t")
    if len(parts) < 8:
        return line
    parts[7] = _add_source_to_info(parts[7], source)
    return "\t".join(parts) + "\n"


def merge_vcfs(vcf_sources, out_vcf: str):
    existing = [(str(v), src) for v, src in vcf_sources if v and Path(v).exists()]
    empty_stats = {"total_records": 0, "deduplicated_records": 0, "gatk_records": 0, "delly_records": 0}
    if not existing:
    ...
    chrom_header = None
    variant_map = {}
    source_counts = {"GATK": 0, "DELLY": 0}
    total_records = 0
    for vcf, source in existing:
        try:
            with _open_maybe_gzip(vcf, 'rt') as fh:
                for line in fh:
                    if line.startswith('##'):
                    ...
                    normalized = _normalize_variant_line(line, source)
                    if key not in variant_map:
                        variant_map[key] = normalized
                    else:
                        old_parts = variant_map[key].rstrip('\n').split('\t')
                        if len(old_parts) >= 8:
                            old_parts[7] = _add_source_to_info(old_parts[7], source)
                            variant_map[key] = '\t'.join(old_parts) + '\n'
        except Exception as e:
            logging.error(f"Failed to read VCF {vcf}: {e}")
            return False, empty_stats
    if chrom_header is None:
        logging.error("VCF merge failed: #CHROM header not found")
        return False, empty_stats
    source_header = '##INFO=<ID=SOURCE,Number=.,Type=String,Description="Variant caller source used during smart merge (GATK, DELLY, or both)">\n'
    if source_header not in seen_headers:
        merged_headers.append(source_header)
    records = list(variant_map.values())
    if not records:
        logging.error("No variant records found while merging VCFs")
        return False, empty_stats
    ...
    try:
        ...
        stats = {
            ...
            "gatk_records": source_counts.get('GATK', 0),
            "delly_records": source_counts.get('DELLY', 0),
        }
        logging.info(
            f"Header-aware smart merge complete: {out_vcf} | total={stats['total_records']} unique={stats['deduplicated_records']} "
            f"GATK={stats['gatk_records']} DELLY={stats['delly_records']}"
        )
        return True, stats
    except Exception as e:
        logging.error(f"Failed writing merged VCF {out_vcf}: {e}")
        return False, empty_stats
...
def _evidence_rows(df, name_set):
    if df.empty or "confidence" not in df.columns:
        return pd.DataFrame()
    resistant_df = df[df["confidence"].map(conf_is_resistant)].copy()
    keep = [c for c in ["drug", "Gene", "Mutation", "original_mutation", "confidence"] if c in resistant_df.columns]
    out = resistant_df[resistant_df["drug"].map(_norm).isin(name_set)][keep].copy()
    if "Mutation" not in out.columns:
        out["Mutation"] = ""
    if "original_mutation" not in out.columns:
        out["original_mutation"] = ""
    out["ShownMutation"] = out["Mutation"].fillna("")
    out.loc[out["ShownMutation"] == "", "ShownMutation"] = out["original_mutation"].fillna("")
    return out.drop_duplicates(subset=["drug", "Gene", "ShownMutation", "confidence"]).reset_index(drop=True)


def predict_final_category(matched_res_csv: str, out_txt: str, out_csv: str):
    ...
    if df.empty:
        ...
        return cat, {"rif": pd.DataFrame(), "inh": pd.DataFrame(), "fq": pd.DataFrame(), "ga": pd.DataFrame()}
    ...
    evid = {
        "rif": _evidence_rows(df, RIF_NAMES),
        "inh": _evidence_rows(df, INH_NAMES),
        "fq": _evidence_rows(df, FQ_NAMES),
        "ga": _evidence_rows(df, GA_EXTRA),
    }

    lines = [f"Final category: {category}", ""]
    if not evid["rif"].empty:
        lines.append("Rifampicin evidence:")
        for _, r in evid["rif"].iterrows():
            lines.append(f"  - {r.get('drug','')} | {r.get('Gene','')} | {r.get('ShownMutation','')} | {r.get('confidence','')}")
        lines.append("")
    if not evid["inh"].empty:
        lines.append("Isoniazid evidence:")
        for _, r in evid["inh"].iterrows():
            lines.append(f"  - {r.get('drug','')} | {r.get('Gene','')} | {r.get('ShownMutation','')} | {r.get('confidence','')}")
        lines.append("")
    if not evid["fq"].empty:
        lines.append("Fluoroquinolone evidence:")
        for _, r in evid["fq"].iterrows():
            lines.append(f"  - {r.get('drug','')} | {r.get('Gene','')} | {r.get('ShownMutation','')} | {r.get('confidence','')}")
        lines.append("")
    if not evid["ga"].empty:
        lines.append("Group A additional evidence:")
        for _, r in evid["ga"].iterrows():
            lines.append(f"  - {r.get('drug','')} | {r.get('Gene','')} | {r.get('ShownMutation','')} | {r.get('confidence','')}")
        lines.append("")
    ...
    return category, evid


class PillBox(Flowable):
    def __init__(self, label, bg, width=60*mm, height=20*mm, font_size=13, text_color=colors.white):
    ...
    def draw(self):
        ...
        c.drawCentredString(self.width/2.0, self.height/2.0 - self.font_size*0.35, self.label[:40])
        c.restoreState()


class DrugVialIcon(Flowable):
    def __init__(self, label, solution_color=colors.HexColor("#4F46E5"), width=28*mm, height=42*mm, subtitle=""):
        Flowable.__init__(self)
        self.label = label
...
class ProgressBar(Flowable):
    def __init__(self, label, value, width=82*mm, height=8*mm):
        Flowable.__init__(self)
        self.label = label
    ...
    def wrap(self, availWidth, availHeight):
        return self.width, self.height + 5*mm

    def draw(self):
        c = self.canv
        c.saveState()
        c.setFillColor(colors.HexColor("#E2E8F0"))
        c.roundRect(0, 0, self.width, self.height, 1.5*mm, stroke=0, fill=1)
        fill_w = self.width * (self.value / 100.0)
        c.setFillColor(colors.HexColor("#2563EB"))
        c.roundRect(0, 0, fill_w, self.height, 1.5*mm, stroke=0, fill=1)
        c.setFont("Helvetica-Bold", 9)
        c.setFillColor(colors.HexColor("#0F172A"))
        c.drawString(0, self.height + 2, f"{self.label}: {self.value:.1f}%")
        c.restoreState()
...
def drug_color_map(drug: str):
    ...
    mapping = {
        ...
        "bedaquiline": "#14B8A6", "linezolid": "#22C55E",
    }
...
def make_summary_cards(sample_id, predicted, mixed, category):
    ...
    label_style = ParagraphStyle('LabelCard', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=9.8, leading=11, textColor=colors.white, alignment=TA_LEFT)
    value_style = ParagraphStyle('ValueCard', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=13.5, leading=16, textColor=colors.white, alignment=TA_LEFT)
    card_w = 62*mm
    card_h = 22*mm

    def card(title, value, bg):
        t = Table([[Paragraph(title, label_style)], [Paragraph(value, value_style)]], colWidths=[card_w], rowHeights=[7*mm, 15*mm])
    ...
    return Table([[
        card("Sample name", sample_id, colors.HexColor("#0F172A")),
        card("Predicted lineage", predicted, colors.HexColor("#2563EB")),
        card("Mixed lineage", mixed, colors.HexColor("#7C3AED")),
        card("Drug resistance", category, resistance_badge_color(category)),
    ]], colWidths=[card_w]*4, rowHeights=[card_h])


def build_visualization_panel(top_lineages):
    bars = [ProgressBar(label, value) for label, value in top_lineages]
    tbl = Table([[b] for b in bars], colWidths=[86*mm])
...
def build_lineage_table(lineage_references, results, matched_counts):
    styles = getSampleStyleSheet()
    hdr = ParagraphStyle('Hdr', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=9.5, textColor=colors.white, alignment=TA_CENTER)
    cell = ParagraphStyle('Cell', parent=styles['Normal'], fontName='Helvetica', fontSize=8.4, leading=9.5)
...
def build_mdr_justification_section(category: str, evid: dict):
    styles = getSampleStyleSheet()
    section_title = ParagraphStyle('SecTitleJ', parent=styles['Heading2'], fontName='Helvetica-Bold', fontSize=14, textColor=colors.HexColor("#0F172A"))
    label = ParagraphStyle('JustLabel', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=10.5, textColor=colors.HexColor("#0F172A"))
    item = ParagraphStyle('JustItem', parent=styles['Normal'], fontName='Helvetica', fontSize=9.6, leading=11)
    body = [Paragraph("Category rule justification", section_title), Spacer(1, 2.5*mm)]
    badge = Table([[PillBox(category, resistance_badge_color(category), width=40*mm, height=12*mm, font_size=9)]], colWidths=[42*mm])
    badge.setStyle(TableStyle([('LEFTPADDING', (0,0), (-1,-1), 0), ('RIGHTPADDING', (0,0), (-1,-1), 0)]))
    body.append(badge)
    body.append(Spacer(1, 2.5*mm))

    sections = [("Rifampicin evidence", evid.get("rif", pd.DataFrame())), ("Isoniazid evidence", evid.get("inh", pd.DataFrame()))]
    if category in {"pre-XDR-TB", "XDR-TB"}:
        sections.append(("Fluoroquinolone evidence", evid.get("fq", pd.DataFrame())))
    if category == "XDR-TB":
        sections.append(("Group A additional evidence", evid.get("ga", pd.DataFrame())))
    ...
    for title, df in sections:
        ...
        for _, r in df.iterrows():
            items.append(Paragraph(f"- {r.get('drug','')} | {r.get('Gene','')} | {r.get('ShownMutation','')} | {r.get('confidence','')}", item))
        rows.append([Paragraph(title, label), Table([[x] for x in items], colWidths=[120*mm])])

    if not rows:
        rows = [[Paragraph("Evidence", label), Paragraph("No resistance-associated evidence detected.", item)]]

    tbl = Table(rows, colWidths=[48*mm, 132*mm])
    tbl.setStyle(TableStyle([
        ('GRID', (0, 0), (-1, -1), 0.25, colors.HexColor("#CBD5E1")),
...
def build_drug_hits_section(matched_res_csv: str, category: str):
    ...
    df = df[df["confidence"].map(conf_is_resistant)].drop_duplicates(subset=["drug", "Gene", "MutShown", "confidence"])

    if df.empty:
        body.append(Paragraph("No resistance-associated antibiotic evidence after filtering.", styles['Normal']))
        return body
    ...
    for _, r in df.iterrows():
        ...
        icon_cells.append(DrugVialIcon(drug[:12], solution_color=drug_color_map(drug), subtitle=gene))
        detail_rows.append([drug, gene, mut, conf])

    cols_per_row = min(4, len(icon_cells)) if icon_cells else 1
    ...
    if icon_rows:
        ...
        flask_tbl = Table(icon_rows, colWidths=[32*mm] * cols_per_row)
    ...
    detail_tbl = Table(detail_rows, colWidths=[40*mm, 28*mm, 48*mm, 62*mm], repeatRows=1)
...
def generate_infographic_pdf(sample_id: str, predicted: str, mixed: str, category: str,
                             ...
                             matched_res_csv: str, output_pdf: str, evid: dict):
    ...
    top_lineages = [(lin, pc) for lin, pc in sorted(results.items(), key=lambda x: x[1], reverse=True) if pc > 0][:2]

    story = []
    story.append(make_summary_cards(sample_id, predicted, mixed, category))
    story.append(Spacer(1, 5*mm))
    if top_lineages:
        story.append(Paragraph("Summary visualization", section_title))
        story.append(Spacer(1, 2*mm))
        story.append(build_visualization_panel(top_lineages))
        story.append(Spacer(1, 5*mm))
    ...
    story.append(Spacer(1, 5*mm))
    story.extend(build_mdr_justification_section(category, evid))
    story.append(Spacer(1, 5*mm))
...
def process_sample(sample_path, ref_genome, output_dir, lineage_references,
    ...
    evid = {"rif": pd.DataFrame(), "inh": pd.DataFrame(), "fq": pd.DataFrame(), "ga": pd.DataFrame()}
    ...
    if vcf_override and Path(vcf_override).exists():
        ...
        if bam_override and Path(bam_override).exists():
            ...
            if kind == "vcf":
                ...
                logging.info(f"[{sample_id}] Auto-detected {kind} FASTQ input")
        ...
        if sorted_bam and (not vcf_lineage or not vcf_resist_final):
            ...
            if skip_delly:
                logging.info(f"[{sample_id}] Skipping Delly because --skip_delly was specified")
            else:
                delly_bcf = str(sample_dir / f"{sample_id}.resist.delly.bcf")
                vcf_resist_delly = str(sample_dir / f"{sample_id}.resist.delly.vcf")
                logging.info(f"[{sample_id}] Running Delly structural variant calling")
                if not run_delly_call(sorted_bam, ref_genome, delly_bcf, vcf_resist_delly, delly_cmd=delly_cmd):
                    logging.warning(f"[{sample_id}] Delly failed; continuing with GATK resistance VCF only")
                    vcf_resist_delly = None

            vcf_resist_final = str(sample_dir / f"{sample_id}.resist.combined.vcf")
            ok, _ = merge_vcfs([(vcf_resist_gatk, "GATK"), (vcf_resist_delly, "DELLY")], vcf_resist_final)
            if not ok:
                logging.warning(f"[{sample_id}] Could not smart-merge GATK + Delly VCFs; using GATK resistance VCF only")
    ...
    if mutations_csv and vcf_resist_final:
        ...
        final_category, evid = predict_final_category(str(res_csv), str(final_txt), str(final_csv))
...
def main():
    ap = argparse.ArgumentParser(
        description="LineageXpress v12: uniform PDF layout, focused resistance display, and category rule justification",
    ...
    for sample in samples:
        process_sample(
            ...
            delly_cmd=args.delly_cmd,
            skip_delly=args.skip_delly,
            make_infographic=(not args.no_infographic),
        )
