            ...
            with _open_maybe_gzip(vcf, 'rt') as fh:
                for line in fh:
                    ...
                    if key not in variant_map:
                        ...
                        new_parts = normalized.rstrip('\n').split('\t')
                        if len(old_parts) >= 8 and len(new_parts) >= 8:
...
def generate_infographic_pdf(sample_id: str, predicted: str, mixed: str, category: str,
    ...
    story.append(Spacer(1, 4*mm))
    story.append(Paragraph("Header-aware smart merge preserves caller-specific header definitions for downstream parsing.", normal))
    doc.build(story)
...
def main():
    ap = argparse.ArgumentParser(
        description="LineageXpress v11: header-aware smart VCF merge plus visualization-rich A3 PDF",
