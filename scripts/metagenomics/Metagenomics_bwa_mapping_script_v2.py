...
def write_lineage_report(sample_id, lineage_references, results, matched_counts, out_path, elapsed_minutes):
    ...
    # Permissive prediction: any lineage with >=1 matched SNP is reported
    positive_lineages = [
        lin for lin in lineage_references
        if matched_counts.get(lin, "0/0").split("/")[0].isdigit() and int(matched_counts.get(lin, "0/0").split("/")[0]) > 0
    ]

    if positive_lineages:
        predicted = ";".join(sorted(positive_lineages))
    elif not sorted_lineages or sorted_lineages[0][1] == 0.0:
    ...
    # Keep mixed lineage stringent as before
...
def process_sample(sample_spec, ref_genome, output_dir, lineage_references, threads=1,
    ...
    write_lineage_report(sample_id, lineage_references, results, matched_counts, result_txt, elapsed_minutes)
    logging.info(f"[{sample_id}] Lineage result saved to {result_txt}")


def main():
    ap = argparse.ArgumentParser(
        description="Permissive metagenomics lineage pipeline: any lineage with >=1 matched SNP is reported",
