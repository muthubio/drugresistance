...
SUMMARY_COLUMNS = [
    "Sample_name",
    "Mycobacterium_percentage_of_host_removed",
    ...
    with opener(vcf_path, mode) as vcf:
        for line in vcf:
            ...
            if len(ref) == 1 and all(len(a) == 1 for a in alt.split(",")):
                dp = None
                mq = None
                bq = qual if qual not in {".", ""} else "NA"
                for item in info.split(";"):
                    if item.startswith("DP="):
                        ...
                        try:
                            ...
                            pass
                try:
                    sample_snps.add((chrom, int(pos), bq, dp, mq))
                except ValueError:
                    pass
    return sample_snps
...
def evaluate_lineages(lineage_references, sample_snps):
    sample_pos_map = {(chrom, pos): {"bq": bq, "dp": dp, "mq": mq} for chrom, pos, bq, dp, mq in sample_snps}
    sample_pos_set = set(sample_pos_map.keys())
    results = {}
    matched_counts = {}
    matched_positions = {}
    for lineage, ref_snps in lineage_references.items():
        matched = sorted(set(ref_snps) & sample_pos_set, key=lambda x: x[1])
        total = len(ref_snps)
...
def summarize_best_snp(best_lineages, matched_positions, sample_pos_map):
    ...
    metrics = sample_pos_map.get(best_pos, {})
    return (
        str(best_pos[1]),
        str(metrics.get("bq", "NA")),
        str(metrics.get("dp", "NA")) if metrics.get("dp") is not None else "NA",
        str(metrics.get("mq", "NA")) if metrics.get("mq") is not None else "NA",
    )


def write_lineage_report(sample_id, lineage_references, results, matched_counts, matched_positions, sample_pos_map, out_path, elapsed_minutes, predicted, best_lineages):
...
def process_sample(sample_spec, ref_genome, output_dir, lineage_references, myco_pct_map, threads=1,
    ...
    predicted, best_lineages = choose_prediction(results, matched_positions, len(sample_snps))
    best_pos, base_qual, depth, mapping_quality = summarize_best_snp(best_lineages, matched_positions, sample_pos_map)

    elapsed_minutes = (time.time() - start_time) / 60
    result_txt = sample_dir / f"{sample_id}_lineage_result.txt"
    write_lineage_report(sample_id, lineage_references, results, matched_counts, matched_positions, sample_pos_map, result_txt, elapsed_minutes, predicted, best_lineages)
    logging.info(f"[{sample_id}] Lineage result saved to {result_txt}")

    return {
        "Sample_name": sample_id,
        "Mycobacterium_percentage_of_host_removed": myco_pct_map.get(sample_id, "NA"),
        "SNP_position_Lineage_express_db": best_pos,
...
def load_myco_percentage_map(summary_csv_path: str):
    mapping = {}
    ...
    with open(summary_csv_path, newline="") as f:
        ...
        for row in reader:
            sample = row.get("Sample") or row.get("Sample_name") or ""
            sample = normalize_sample_id(sample)
            pct_val = row.get("Mycobacterium_pct_of_host_removed") or row.get("Mycobacterium_percentage_of_host_removed") or "NA"
            mapping[sample] = pct_val
    return mapping


def write_summary_csv(rows, output_dir, filename="metagenomics_lineage_summary.csv"):
    out_path = Path(output_dir) / filename
    with open(out_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=SUMMARY_COLUMNS)
        writer.writeheader()
        for row in rows:
            writer.writerow({col: row.get(col, "NA") for col in SUMMARY_COLUMNS})
    logging.info(f"Summary CSV written to {out_path}")
    return out_path


def main():
    ap = argparse.ArgumentParser(
        description="Metagenomics lineage pipeline with final summary CSV output",
    ...
    ap.add_argument("--summary_input_csv", default=None,
                    help="Optional input CSV containing Mycobacterium percentage column")
    ...
    myco_pct_map = load_myco_percentage_map(args.summary_input_csv)
    ...
    for sample in samples:
        row = process_sample(sample, args.ref_genome, args.output_dir, lineage_refs, myco_pct_map,
                             args.threads, args.bam_file, args.vcf_file,
                             args.min_bq, args.min_mq, args.max_depth)
        if row:
            summary_rows.append(row)

    write_summary_csv(summary_rows, args.output_dir, args.summary_output_csv)
