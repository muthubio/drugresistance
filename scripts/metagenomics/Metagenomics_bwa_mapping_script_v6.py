...
def count_fastq_reads(fq_path: str) -> int:
    import gzip
    opener = gzip.open if str(fq_path).endswith(".gz") else open
    n = 0
    with opener(fq_path, "rt", errors="replace") as fh:
        for _ in fh:
            n += 1
    return n // 4


def compute_mycobacterium_percentage(host_removed_inputs, mtb_fastqs, input_type):
    try:
        if input_type == "paired":
            host_removed_reads = count_fastq_reads(host_removed_inputs[0])
            mtb_reads = count_fastq_reads(mtb_fastqs[0])
        else:
            host_removed_reads = count_fastq_reads(host_removed_inputs[0])
            mtb_reads = count_fastq_reads(mtb_fastqs[0])
        if host_removed_reads == 0:
            return "NA"
        return round((mtb_reads / host_removed_reads) * 100, 6)
    except Exception:
        return "NA"
...
def process_sample(sample_spec, ref_genome, output_dir, lineage_references, threads=1,
    ...
    myco_pct = "NA"
    if inputs and kind in {"paired", "single"}:
        myco_pct = compute_mycobacterium_percentage(inputs, inputs, kind)
    ...
    return {
        ...
        "Mycobacterium_percentage_of_host_removed": myco_pct,
...
def main():
    ap = argparse.ArgumentParser(
        description="Metagenomics lineage pipeline with automatic Mycobacterium percentage calculation",
    ...
    for sample in samples:
        row = process_sample(sample, args.ref_genome, args.output_dir, lineage_refs,
