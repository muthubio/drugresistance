...
def clean_sample_name(filename):
    ...
    patterns = [
        r"_host_removed_R1$", r"_host_removed_R2$", r"_host_removed_1$", r"_host_removed_2$",
        r"_R1_001$", r"_R2_001$", r"_R1$", r"_R2$", r"_r1$", r"_r2$", r"_1$", r"_2$",
        r"\.R1$", r"\.R2$", r"\.r1$", r"\.r2$", r"_out1$", r"_out2$", r"\.1$", r"\.2$"
    ]
    for p in patterns:
        base = re.sub(p, "", base)
    return base
...
def get_mycobacterium_percentage_from_kraken_report(report_file):
    patterns = ["mycobacterium", "tuberculosis", "africanum", "bovis", "caprae", "canettii", "microti", "pinnipedii", "orygis", "mungi"]
    total = 0.0
    with open(report_file, "r", errors="replace") as fh:
        for line in fh:
...
def parse_vcf_variants(vcf_file):
    ...
    for line in lines.splitlines():
        ...
        if len(fields) < 8:
            continue
        chrom = contig_alias_to_nc(fields[0])
        try:
            pos = int(fields[1])
        except ValueError:
            continue
...
def lineage_prediction_from_vcf(vcf_file, snp_db, bed_file):
    ...
    for lineage, ref_snps in lineage_refs.items():
        ...
        matched_counts[lineage] = matched_count
        results[lineage] = round((matched_count / total) * 100, 2) if total > 0 else 0.0
    ...
    top_lineage, top_prob = sorted_lineages[0]
    top_matched = matched_counts.get(top_lineage, 0)

    if top_prob == 0.0 or top_matched == 0:
        predicted = "Unpredictable: insufficient SNP evidence"
    else:
        predicted = top_lineage

    matched_positions = matched_positions_per_lineage.get(top_lineage, [])
    if not matched_positions or predicted.startswith("Unpredictable"):
    ...
    best_pos = matched_positions[0]
    st = stats_by_pos.get(best_pos, {})
    return {
        "predicted_lineage": predicted,
        "matched_snp_positions": str(best_pos[1]),
        "representative_bq": str(st.get("BQ", "NA")) if st.get("BQ", ".") not in [".", ""] else "NA",
        "representative_dp": str(st.get("DP", "NA")) if st.get("DP", ".") not in [".", ""] else "NA",
        "representative_mq": str(st.get("MQ", "NA")) if st.get("MQ", ".") not in [".", ""] else "NA",
    }
...
def process_sample(entry, args):
    ...
    run_command(cmd)
    myco_pct = get_mycobacterium_percentage_from_kraken_report(kraken_report)
...
def main():
    p = argparse.ArgumentParser(description="Run lineage prediction from host-removed FASTQ reads (haploid bcftools calling, v3 aligned with v5)")
