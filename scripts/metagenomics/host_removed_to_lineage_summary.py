...
PAIR_PATTERNS = [
    ...
    ("_out1", "_out2"),
    (".1", ".2"),
    ("_host_removed_R1", "_host_removed_R2"),
    ("_host_removed_1", "_host_removed_2"),
]
...
def clean_sample_name(filename):
    ...
    patterns = [
        r"_host_removed_R1$", r"_host_removed_R2$",
        r"_host_removed_1$", r"_host_removed_2$",
        r"_R1_001$", r"_R2_001$",
        r"_R1$", r"_R2$",
        r"_r1$", r"_r2$",
        r"_1$", r"_2$",
...
def lineage_prediction_from_vcf(vcf_file, snp_db, bed_file):
    ...
    for lineage, ref_snps in lineage_refs.items():
        ...
        total = len(ref_snps)
        prob = round((len(matched) / total) * 100, 2) if total > 0 else 0.0
    ...
    return {
        "predicted_lineage": predicted,
        "matched_snp_positions": ",".join(pos_strings) if pos_strings else "NA",
        "representative_bq": ",".join(bqs) if bqs else "NA",
        "representative_dp": ",".join(dps) if dps else "NA",
        "representative_mq": ",".join(mqs) if mqs else "NA",
    }
...
def extract_mtb_reads(input_type, original_fastq_files, kraken_out, kraken_report, extract_script, outdir, sample_name):
    ...
    if input_type == "paired":
        ...
        open(myco_r1, "w").close()
        open(myco_r2, "w").close()
        for taxid in taxids:
            ...
            cmd = ["python3", extract_script, "-k", kraken_out, "-r", kraken_report, "-s", original_fastq_files[0], "-s2", original_fastq_files[1], "-t", str(taxid), "--include-children", "--fastq-output", "-o", out_r1, "-o2", out_r2]
            run_command(cmd)
            append_file(out_r1, myco_r1)
            append_file(out_r2, myco_r2)
        ...
        if os.path.getsize(myco_r1) > 0 and os.path.getsize(myco_r2) > 0:
            ...
            os.replace(dedup_r1, myco_r1)
            os.replace(dedup_r2, myco_r2)
    ...
    for taxid in taxids:
        ...
        cmd = ["python3", extract_script, "-k", kraken_out, "-r", kraken_report, "-s", original_fastq_files[0], "-t", str(taxid), "--include-children", "--fastq-output", "-o", out_se]
        run_command(cmd)
...
def bcftools_call(ref, bam, outdir, sample_name):
    ...
    vcfgz = f"{vcf}.gz"
    mpileup_cmd = f"bcftools mpileup -Ou -f {shlex.quote(ref)} {shlex.quote(bam)} | bcftools call -mv -Ov -o {shlex.quote(vcf)}"
...
def process_sample(entry, args):
    ...
    ensure_dir(sample_res)
    ensure_dir(kraken_dir)
    ensure_dir(mtb_dir)
    ...
    return {
        ...
        "SNP_position_Lineage_express_db": lineage["matched_snp_positions"],
        "Base_quality": lineage["representative_bq"],
        "Depth": lineage["representative_dp"],
        "Mapping_Quality": lineage["representative_mq"],
        "Predicted_lineage": lineage["predicted_lineage"],
    }


def main():
    parser = argparse.ArgumentParser(description="Run lineage prediction from host-removed FASTQ reads")
    parser.add_argument("--input", help="Single sample input: FASTQ file, paired FASTQ as R1,R2, or sample prefix path")
    parser.add_argument("--input_list", help="Text file with one sample entry per line")
    parser.add_argument("--input_dir", help="Directory containing host-removed FASTQ files for automatic sample detection")
    parser.add_argument("--ref", default="/home/muthukumarb/drugresistance/data/reference/h37rv.fa", help="H37Rv reference FASTA")
    parser.add_argument("--snp_db", default="/home/muthukumarb/drugresistance/data/database/lineage_snp_updated_au13.tsv", help="Lineage SNP database")
    parser.add_argument("--bed_file", default="/home/muthukumarb/drugresistance/data/database/targeted_modified_regions_au13.bed", help="Targeted BED file")
    parser.add_argument("--kraken_db", required=True, help="Kraken2 database")
    parser.add_argument("--extract_script", required=True, help="Path to KrakenTools extract_kraken_reads.py")
    parser.add_argument("--tmp_dir", default="tmp_folder", help="Intermediate output directory")
    parser.add_argument("--result_dir", default="result_folder", help="Final result directory")
    parser.add_argument("--threads", type=int, default=4, help="Threads")
    parser.add_argument("--kraken_memory_mapping", action="store_true", help="Use --memory-mapping in Kraken2")
    parser.add_argument("--summary_csv", default="final_lineage_summary.csv", help="Final summary CSV name")
    args = parser.parse_args()

    provided = [bool(args.input), bool(args.input_list), bool(args.input_dir)]
    if sum(provided) != 1:
        raise ValueError("Provide exactly one of --input, --input_list, or --input_dir")

    required_paths = {"ref": args.ref, "snp_db": args.snp_db, "bed_file": args.bed_file, "kraken_db": args.kraken_db, "extract_script": args.extract_script}
    ...
    if args.input_list:
        ...
        for fname in sorted(os.listdir(args.input_dir)):
            ...
            if not os.path.isfile(full):
                continue
            if not any(fname.endswith(ext) for ext in FASTQ_EXTENSIONS):
                continue
    ...
    for entry in samples:
        rows.append(process_sample(entry, args))

    out_csv = os.path.join(args.result_dir, args.summary_csv)
