...
def match_catalogue(ann_csv: str, mutations_csv: str,
    ...
    # Resistance-only
    if "confidence" in out_all_df.columns:
        out_res_df = out_all_df[out_all_df["confidence"].map(conf_is_resistant)].copy()
    else:
        out_res_df = out_all_df.iloc[0:0].copy()

    if out_res_df.empty:
        # SAFELY get confidence series (may not exist or may be empty)
        conf_series = out_all_df.get("confidence", pd.Series([], dtype="object"))
        if not conf_series.empty:
            _conf_vals = (
                conf_series.astype(str)
                .str.lower()
                .value_counts()
                .head(10)
            )
            logging.info(
                "resistance filter: 0 rows passed. Top confidence values:\n%s",
                _conf_vals.to_string()
            )
        else:
            logging.info("resistance filter: 0 rows passed. No 'confidence' values present.")
...
def write_drug_summary(matched_res_path: str, out_csv: str):
    try:
        ...
        logging.info("No matched_resistance.csv to summarize.")
        return
    if df.empty:
        logging.info("matched_resistance.csv is empty; no summary created.")
        return
...
def _present(df, name_set): return df["drug"].map(_norm).isin(name_set).any()
def _evidence_rows(df, name_set, cols=("drug","Gene","Mutation","original_mutation","confidence")):
    cols = [c for c in cols if c in df.columns]
    return (df[df["drug"].map(_norm).isin(name_set)][cols]
...
def predict_final_category(matched_res_csv: str, out_txt: str, out_csv: str) -> str:
    ...
    if df.empty:
        cat = "Susceptible/No resistance-associated mutations detected"
        Path(out_txt).write_text(cat + "\n", encoding="utf-8")
        pd.DataFrame({"category":[cat]}).to_csv(out_csv, index=False)
        logging.info(cat); return cat
    if "drug" not in df.columns:
        raise SystemExit("matched_resistance.csv has no 'drug' column")
    ...
    if (has_rif or has_inh) and has_fq and has_ga:
        ...
        category = "Susceptible/No resistance-associated mutations detected"
...
def process_sample(sample_path, ref_genome, output_dir, lineage_references,
    ...
    if mutations_csv:
        ...
        res_csv = sample_dir / "matched_resistance.csv"
        res_summary_csv = sample_dir / "matched_resistance_summary.csv"
        final_txt = sample_dir / f"{sample_id}_final_prediction.txt"
        final_csv = sample_dir / f"{sample_id}_final_prediction.csv"
        ...
        try:
            ...
            logging.info(f"[{sample_id}] matched_resistance rows: {_df_dbg.shape[0]}  path: {res_csv}")
            if "drug" in _df_dbg.columns:
                logging.info(f"[{sample_id}] drugs seen: {sorted(set(str(x).lower() for x in _df_dbg['drug']))}")
        except Exception as e:
            logging.warning(f"[{sample_id}] could not preview matched_resistance: {e}")
